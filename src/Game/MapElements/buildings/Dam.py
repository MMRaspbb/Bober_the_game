...
class Dam(AbstractBuilding, Rotatable):
    ...
    def get_representation(self) -> list:
        representation = []
        img = pygame.image.load("src/resources/dam.png")
        ...
        if self.get_orientation() == MapDirection.NORTH or self.get_orientation() == MapDirection.SOUTH:
            ...
            representation = [[self.position[0], self.position[1], img],
                            [self.position[0] + 1, self.position[1], img],
                            [self.position[0] - 1, self.position[1], img],
                            [self.position[0], self.position[1] + 1, img],
                            [self.position[0], self.position[1] - 1, img],
                            [self.position[0] + 1, self.position[1] + 1, img],
                            [self.position[0] - 1, self.position[1] - 1, img],
                            [self.position[0] + 2, self.position[1] + 2, img],
                            [self.position[0] - 2, self.position[1] - 2, img],
                            [self.position[0] + 2, self.position[1] + 1, img],
                            [self.position[0] - 2, self.position[1] - 1, img],
                            [self.position[0] + 1, self.position[1] + 2, img],
                            [self.position[0] - 1, self.position[1] - 2, img]]
        else:
            representation = [[self.position[0], self.position[1], img],
                            [self.position[0], self.position[1] + 1, img],
